...
本地实现更新（2026-09-17）：参考 True Aire 与 TatFans 的文字、图片入场方式，为全部 14 个已有页面接入共享动效层。

- 首页文字依次错峰进入，主风机图缩放归位，并播放一次产品展示扫描；证据图片与产品卡片按局部列表依次出现，局部错峰间隔为 100ms、最多延迟 300ms。
- 产品、合作伙伴、About 与 Contact 页面随滚动呈现标题、图文和内容组；隐私、状态说明和 WhatsApp 二维码使用更轻的淡入。
- 普通入场为桌面 950ms／手机 700ms，文字和普通内容组位移 40px／22px；产品卡片、分类卡片和证据卡片位移 64px／36px，并从 0.96／0.98 倍缩放归位；轻淡入为 460ms。首页主图入场为桌面 1150ms／手机 850ms、延迟 160ms，单次扫描为 1250ms、延迟 320ms。展示入场使用 `cubic-bezier(0.22, 0.68, 0.36, 1)`，滚回已读区域不重复播放。
- 精确鼠标设备上的产品及分类卡片在 260ms 内上移 8px，图片在 300ms 内放大至 1.075 倍，卡片内箭头右移 6px；按钮保持 200ms、上移 1px，独立文字链接箭头保持 180ms、右移 3px。交互反馈继续使用 `cubic-bezier(0.16, 1, 0.3, 1)`。
- 内容默认可见，不依赖 JavaScript 才能阅读。键盘焦点、锚点和打印即时显示相关内容；系统减少动态效果时跳过展示动效并保留轻微颜色反馈。RFQ 控件、校验错误、Turnstile 与隐藏的买家面板不参与入场动画。

实现入口为 `src/scripts/motion.ts`，通过 `BaseLayout.astro` 全站加载，使用原生 Web Animations API 与 IntersectionObserver，没有增加依赖。页面文案、图片资产、整卡链接和询价协议保持原有内容。

首次动效版本的本地检查：`npm run build` 通过，24 个文件零错误／警告／提示并生成 14 个页面；Worker 类型检查、63 项 RFQ 测试和 12 项 CI 测试通过。Wrangler 类型一致性检查在默认沙箱内报告过期，改用更广只读环境检查后通过，未重新生成类型或修改配置。14 页的 1440px 桌面及 390px 手机浏览器检查未发现横向溢出或已加载图片失败，询价控件未被动画包裹。最终节奏与幅度调整后再次构建通过，桌面／手机定向检查确认卡片入场、8px 悬停上移、1.075 倍图片缩放、整卡 Enter 导航及二维码锚点正常。详细记录位于被 Git 忽略的 `.impeccable/review/motion/REVIEW.md`。

用户已于 2026-09-17 验收最终动效，并通过“执行命令一”授权将本次动效与设计说明选择性提交、推送至 `origin/main`，按现有 Workers Builds 连接触发自动构建部署。Git 同步与本地检查不证明对应版本已成功上线；线上效果以该提交的部署结果为准。本次发布范围不包含后端、生产配置或真实邮件测试。

动效命令一发布前复核（2026-09-17）：`npm run build` 与完整 `npm run check:ci` 均通过；Astro 检查 24 个文件零错误／警告／提示并生成 14 页，CI 包含 Worker 类型检查、63 项 RFQ 测试、12 项 Node CI 测试与 Wrangler 类型一致性检查。CI 在更广文件读取权限下通过，日志写入项目内被忽略的 `.wrangler/logs/`，未重新生成源码类型或修改配置。

### 2A Precision 品牌标志

用户于 2026-09-20 确认采用 `2A / Precision` 作为 XINGXU FAN 正式标志。生产版本按确认方案重新绘制为三块纯色矢量几何：深蓝主斜笔、深蓝左下段和橙色右上段，保留透明间隙与锐角，不使用概念展示图中的白底、渐变或阴影。

- 页眉使用紧凑的图形与 `XINGXU FAN` 横向组合；手机端隐藏副标题，保留图形和品牌字标。
- 深色页脚使用白色与橙色反向版本，链接继续保留明确的首页可访问名称。
- `src/assets/brand/` 保存标准、反白、独立图标和横向锁定式 SVG 母稿及使用说明。
- `public/` 提供 SVG favicon、32×32 PNG favicon 和带安全留白的 180×180 Apple Touch 图标。
- `BaseLayout.astro` 统一输出浏览器图标链接，全部 14 个现有页面共享同一品牌入口。

本地验收（2026-09-20）：Astro 检查与构建通过，25 个文件零错误／警告／提示并生成 14 个页面；Worker 类型检查、63 项 Vitest、12 项 Node CI 测试与 Wrangler 类型一致性检查通过。1440px 桌面和窄屏预览确认页眉没有挤压或横向溢出，桌面页脚反白版本显示正常。概念探索稿保留在本地 `output/`，不进入源码提交或网站构建。
...
- WhatsApp 使用用户提供的 LeoChen 二维码原图，完整保存在 `src/assets/contact/leochen-whatsapp-qr.jpg`；联系页按比例展示并支持打开原图，其他页面入口指向 `/contact/#whatsapp`。不重绘二维码，不猜测电话号码或生成 WhatsApp 直达链接。
- Facebook、WeChat 继续标注联系资料待补充。

本次更新首页、联系页、全站页脚、询盘页与隐私页的公开联系信息，并由共享数据维护邮箱。公开邮箱展示不更改 RFQ 实际收件目标、邮件绑定、Runtime Secrets 或生产配置，也不代表真实邮件投递或 Gmail 收件已验收；隐私页仅更新联系信息，不扩写保留期限或权利处理承诺。用户随后于 2026-09-15 通过“执行命令一”授权本次选择性提交与推送到 `origin/main`，按现有 Workers Builds 连接触发自动构建部署；部署结果须单独核验，该授权不包含额外 Cloudflare 配置变更或真实邮件测试。

本地验证（2026-09-15）：Astro 检查 23 个文件零错误／警告／提示，构建 14 个页面；Worker 类型检查、63 项 RFQ 测试、12 项 CI 脚本测试通过。Wrangler 类型一致性检查在默认沙箱内因 esbuild 无法读取父目录而误报过期，改用更广的只读权限后通过，未重新生成源码类型或修改配置。首页、联系页、询盘页、隐私页和 Products 在 1440、768、390、320px 共 20 组浏览器检查通过，未发现页面或联系区域溢出、失败图片或不一致的邮件链接；二维码按原比例显示，原图打开和询盘页跳转二维码区域均通过。上传文件、源码图片、构建产物及 HTTP 返回图片的 SHA-256 一致。实体手机／WhatsApp 扫码未执行，仍需用户确认；浏览器证据位于被 Git 忽略的 `.impeccable/review/contact-update/`。

命令一发布前复核（2026-09-15）：`npm run build` 与完整 `npm run check:ci` 均通过；Wrangler 日志写入项目内被忽略的 `.wrangler/logs/`，类型检查使用已核准的更广文件读取权限。
...
- 不提交本地环境文件、真实 Secret、未公开的 RFQ 收件目标邮箱、本地依赖、构建产物或本机工具状态；仓库可保留用户授权展示的公开联系邮箱，以及明确列出的非敏感配置名与示例值。
